...
- 多仓库代码变更继续增强：当一个项目目录下包含多个 Git 子仓库时，代码变更面板新增仓库维度的多选过滤，默认全选，用户可以只查看自己关心的仓库，不再被所有子仓库文件一起淹没。
- 为“历史脏仓库 / 超大变更仓库”增加保护：当跨仓库变更文件过多时，后端不再默认一次性展开全部文件明细，而是先返回仓库摘要，并提示用户先缩小到目标仓库后再查看文件列表和 patch，避免直接触发 `git diff 计算失败`。
- 代码变更接口补齐多仓库下钻能力：前端现在会把选中的多个仓库传给后端，统计、文件列表和单文件 patch 都按当前仓库选择范围返回，保留默认全选的使用习惯，同时显著降低大工作区的计算压力。

## 0.1.43

- 修复手机端“代码变更”弹窗的滚动结构：代码变更面板改为整体正文可滑动，上方范围切换、仓库摘要、告警与移动端 tab 不再长期固定，避免下面真正可滚动区域只剩一小条。
- 同步调整移动端代码变更弹窗容器的滚动策略，减少多层嵌套滚动带来的卡顿和误触，文件列表与 diff 预览在小屏上有更稳定的可视高度。

## 0.1.42

- 飞书通知从简单文本消息升级为交互卡片，补齐状态头部、项目/引擎/时间信息区、摘要/变更/验证等结构化分区，并新增更易读的 Markdown/列表展示逻辑，避免长摘要糊成一整段。
- 继续打磨飞书通知细节：时间改为更紧凑的单独一行展示；Markdown 列表、风险与下一步提示会优先按区块拆开；失败、成功等状态会通过卡片头部颜色进行区分。
- 任务页新增“文件”入口，与“代码变更”并列进入同一个检查弹窗；弹窗支持在“代码变更 / 文件”之间切换，不再只能看 diff 范围内的文件。
- 新增任务内项目文件浏览器：支持浏览当前任务绑定项目的完整文件树、展开/折叠目录、预览文本文件内容、对二进制文件给出不可预览提示，并对大文件显示截断提示。
- 文件浏览器继续补齐预览体验：Markdown 文件会按现有渲染链路直接渲染；普通文本文件保持纯文本预览；目录树默认不展开，也不再恢复上一次展开状态，避免首次打开时整棵树全部铺开。
- 修复手机端“选择工作目录”弹窗的滚动结构：正文区域改为整体单层滚动，上方“当前选中 / 搜索 / 新建目录 / tab”不再把目录树挤到只剩一小行可视区域。
- OpenSpec 新增 `add-task-file-browser` 变更提案与 spec，用于定义“任务内项目文件浏览器”能力；同时补充对应服务端测试与实现任务清单，方便后续继续迭代。
